...
1. Отредактируйте строку в `docker-compose`

```
- ../vault_diploma:/vault_code
```

Чтобы `../vault_diploma` соответствовало пути, где лежит код

2. Создайте файл `.env`. Пропишите в нем:

```
VAULT_PATH="/vault_code"
```

3. Запустите компиляцию: `docker-compose up --build`
