...
Перед бэкапом закоммитьте или спрячьте важные локальные изменения: `git bundle` сохраняет историю, но не незакоммиченные файлы. Подробнее: [Резервное копирование](/backup/).
