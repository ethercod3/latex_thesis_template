...
!!! tip "Проверка окружения"
    После установки инструментов можно запустить `task check`. Команда проверит доступность программ, `uv`-окружения и PyLuaTeX, а затем покажет, что уже готово и что нужно установить.
