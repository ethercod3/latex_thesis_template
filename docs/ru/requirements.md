...
!!! tip "Проверка окружения"
    После установки инструментов можно запустить `task check`. Команда проверит входные инструменты для Docker-first сборки и состояние проекта, а локальные инструменты для ручной сборки покажет как опциональные.
