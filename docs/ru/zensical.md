...
Документация проекта собирается через Zensical и лежит в `docs/`. Русская версия находится в `docs/ru/`, английская - в `docs/en/`.
...
Русский сайт собирается из `zensical.toml` в `docs-site/`. Английский сайт собирается из `zensical.en.toml` в `docs-site/en/`. В обоих конфигах задан `extra.alternate`, поэтому при обычном `task docs` в UI появляется переключатель языка.
