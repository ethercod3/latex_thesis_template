...
Если вы используете `latexmk`, вручную повторять компиляцию не нужно: он сам запускает `lualatex` и `biber` столько раз, сколько требуется. В этом проекте это еще и быстрее: около 18 секунд через `latexmk` против примерно 53 секунд через `--no-latexmk`. Повторные ручные запуски нужны только при полностью ручной компиляции из раздела [Сборка без Docker](manual-build.md#полностью-ручная-компиляция). Для скрипта `scripts/build_latex_manual.py` старый ручной режим включается флагом `--no-latexmk`.
